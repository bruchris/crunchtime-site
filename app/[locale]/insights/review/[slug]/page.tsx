...
export async function generateMetadata({
  ...
  if (!previewReviewEnabled()) return { title: "Insight preview" };

  const { locale, slug } = await params;
  const post = await safeGet(slug, locale);
  if (!post) return { title: "Insight preview" };

  const previewPath = `/${locale}/insights/review/${slug}`;
  const languages: Record<string, string> = { [locale]: previewPath };
  if (post.pairSlug) {
    const otherLocale = locale === "no" ? "en" : "no";
    languages[otherLocale] = `/${otherLocale}/insights/review/${post.pairSlug}`;
  }

  return {
    title: `${post.title} (Review Preview)`,
    description: post.excerpt || post.subtitle,
    alternates: { canonical: previewPath, languages },
    robots: { index: false, follow: false, googleBot: { index: false, follow: false } },
    openGraph: {
      url: `${SITE_URL}${previewPath}`,
      title: post.title,
      description: post.excerpt || post.subtitle,
      type: "article",
...
export default async function InsightReviewPost({
  params
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  if (!previewReviewEnabled()) notFound();
...
async function safeGet(slug: string, locale: Locale) {
  try {
    return await getInsightBySlug(slug, locale, { allowedStatuses: ["in-review"] });
  } catch (err) {
    console.error("[insights] failed to fetch review post", { slug, locale, err });
    return null;
  }
}
